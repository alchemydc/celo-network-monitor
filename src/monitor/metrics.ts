export default class Metrics {
    ...
    async logUnique(uniqueKey: string, key: string, value:number, identifier?:string): Promise<void> {
    ...
    async log(key: string, value:number, identifier?:string): Promise<void> {
        if (identifier==undefined) {
            console.log(`namespace="${this.#namespace}" key="${key}", value=${value}`)
        } else {
            console.log(`namespace="${this.#namespace}" key="${key}" value=${value} identifier="${identifier}"`)
        }
    }
}