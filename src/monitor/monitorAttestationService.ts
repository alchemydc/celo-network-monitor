...
export default class MonitorAttestationService extends MonitorBase {
    protected async run() {
        ...
        for (const address of this.addresses.addresses().keys()) {
            ...
            const hasAuthorizedAttestationSigner = await accounts.hasAuthorizedAttestationSigner(address)
            if (!hasAuthorizedAttestationSigner) {
                ...
                await this.alert.slackError(message, 60 * 5, alertKey)
            ...
            console.log(`Attestation Service Monitoring returned ${response.statusCode}: ${response.content}`)
