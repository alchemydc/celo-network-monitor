    ...
    protected async run() {
        ...
        for (const group of this.addresses.groups()) {
            ...
            if (groupVotes.eligible) {
                ...
                console.debug(`> ${weiToIntegerFloorCommas(votes)}/${weiToIntegerFloorCommas(totalCapacity)} Votes. Room for ${weiToIntegerFloorCommas(remainingCapacity)} more votes`)
                this.metrics.log("VoteTotalCapacity", weiToIntegerFloor(totalCapacity), this.addresses.alias(group))
                this.metrics.log("VoteFreeCapacity", weiToIntegerFloor(remainingCapacity), this.addresses.alias(group))
                this.metrics.log("Votes", weiToIntegerFloor(votes), this.addresses.alias(group))
                ...
                if (voteCache.has(group)) {
                    ...
                    if (voteCache.get(group) != votes) {
                        ...
                        if (deltaVotes.isGreaterThan(minDelta)) {
                            const deltaString = votes.isGreaterThan(cachedVotes) ? "increased" : "decreased"

                            await this.alert.slack(
                                `Votes for \`${this.addresses.alias(group)}\` have ${deltaString} by \`${weiToIntegerFloorCommas(deltaVotes)}\` to \`${ weiToIntegerFloorCommas(votes)}\` ${slackAddressDetails(group)}`
                            );
                        }
                    }
                }
