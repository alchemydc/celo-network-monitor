...
export default class MonitorElectabilityThreshold extends MonitorBase {
    protected async run() {
        ...
        for(const group of this.addresses.groups()) {
            const totalGroupVotes = (await election.getTotalVotesForGroup(group)).dividedBy(1e18).toNumber()
            if (totalGroupVotes > 0) {
                this.metrics.log("GroupVotes", totalGroupVotes, this.addresses.alias(group))
                // Votes for last (5th) validator
                const votesForLastValidator = totalGroupVotes / 5
                this.metrics.log("VotesForLastValidator", votesForLastValidator, this.addresses.alias(group))
                // Excess votes
                if (votesForLastValidator > electabilityThreshold) {
                    const excessVotesFor5thValidator = Math.min((votesForLastValidator-electabilityThreshold)*5, totalGroupVotes)
                    this.metrics.log("ExcessGroupVotes", excessVotesFor5thValidator, this.addresses.alias(group))
                }
            }
        }
    }
